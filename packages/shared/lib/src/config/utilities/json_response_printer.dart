...
class JsonResponsePrinter extends Interceptor {
  /// Pretty print Dio logger
  JsonResponsePrinter({
  ...
  void Function(Response<dynamic>) get printResponse => _printResponse;
  ...
  /// Default max width size for pretty print
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        ...
        if (compact && _canFlattenList(value)) {
          logPrint('║${_indent(tabCount)} "$key": $value');
        } else {
          logPrint('║${_indent(tabCount)} "$key": [');
        ...
        if (msg.length + indent.length > linWidth) {
          ...
          for (var i = 0; i < lines; ++i) {
            logPrint('║${_indent(tabCount)} ${msg.substring(
              i * linWidth,
  ...
  void _printMapAsTable(Map<dynamic, dynamic> map, {String? header}) {
    ...
    map.forEach(
      (dynamic key, dynamic value) =>
          _printKV(key.toString(), value.toString()),
    );
    _printLine('╚');
  }
}
